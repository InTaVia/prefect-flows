...
@task
def add_data_from_url(endpoint, source_url, target_graph_uri):
    ...
    #res = requests.post(post_url, headers={'Content-type': 'text/turtle'}, data=f, auth=auth)
    res = requests.post(post_url, headers={'Content-type': 'text/turtle'}, data=res.text.encode('utf-8'))
    logger.info(res)
...
with Flow("Ingest workflow") as flow:    
    ...
    res = add_data_from_url(endpoint, source_url, target_named_graph_uri)
