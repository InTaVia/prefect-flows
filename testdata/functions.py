...
def read_data(sources):
    pldf = pd.read_csv(sources["persons.csv"], sep=",", quotechar='"', encoding='utf-8')
    """Read CSV in Pandas Dataframe."""
    chodf = pd.read_csv(sources["cho.csv"], sep=",", quotechar='"', encoding='utf-8')
...
def write_graph(dir, g):
    ...
    with open(dir + 'exdataset.rdf', 'w') as f:
        f.write(str(exttl))
